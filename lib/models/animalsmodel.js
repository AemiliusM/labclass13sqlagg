...
export default class AnimalsClass {
  ...
  static async animalCount() {
    const { rows } = await pool.query('SELECT species_name, COUNT(*) AS animals FROM animals LEFT JOIN species ON animals.species_id = species.id GROUP BY species.species_name');
